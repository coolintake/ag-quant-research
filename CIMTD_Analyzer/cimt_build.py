...
COMMODITY_CONFIG = {
    ...
    "Canola Oil": {
        "hs6_codes": [151411, 151419],   # crude + refined low-erucic acid rape/canola oil
        "crop_year_start_month": 8,
        "top_destinations": 10,
    },
    "Canola Meal": {
        "hs6_codes": [230641],           # oilcake/solid residues of low-erucic acid rape
        "crop_year_start_month": 8,
        "top_destinations": 10,
    },
    "Soybean Oil": {
        "hs6_codes": [150710, 150790],   # crude + other soybean oil
        "crop_year_start_month": 9,
        "top_destinations": 10,
    },
    "Soybean Meal": {
        "hs6_codes": [230400],           # oilcake and solid residues of soybeans
        "crop_year_start_month": 9,
        "top_destinations": 10,
    },
    "Oats": {
        # Raw oats: 100400 = legacy pre-2012 single code, 100410/100490 = post-2011 split
        # 110312 (expired 2001) and 110319 (non-oat catch-all) removed — no usable volume
        # Processed: 110412 (rolled/flaked, TNE) + 110422 (hulled/pearled, KGM)
        # Chapter 11 codes scaled x1.65 GE via hs6_ge_multipliers
        "hs6_codes": [100400, 100410, 100490, 110412, 110422],
        "crop_year_start_month": 8,
        "top_destinations": 10,
        "hs6_ge_multipliers": {
            110412: 1.65,   # rolled/flaked oats (TNE)
            110422: 1.65,   # hulled/pearled oats (KGM — pipeline divides by 1,000,000)
        },
    },
}
...
# =============================================================================
# GRAIN EQUIVALENT MULTIPLIER
# =============================================================================

def apply_ge_multipliers(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    """
    Applies per-HS6 Grain Equivalent (GE) multipliers to KMT.

    Processed commodities (e.g. oat products, Chapter 11) trade at product
    weight, which understates the raw grain equivalent consumed. The 1.65x
    multiplier converts product KMT back to grain-in equivalent, matching
    AAFC/StatsCan S&D balance sheet methodology.

    GE adjustment is applied AFTER UOM conversion so units are always KMT
    before and after. All outputs (pivot, destinations, Raw Data sheet) use
    the GE-adjusted figure for consistency.

    Commodities without 'hs6_ge_multipliers' in their config are unaffected.
    """
    ge_map = cfg.get("hs6_ge_multipliers")
    if not ge_map:
        return df
    df = df.copy()
    for hs6_code, multiplier in ge_map.items():
        mask = df["HS6"].astype("Int64") == hs6_code
        if mask.any():
            df.loc[mask, "KMT"] *= multiplier
            print(f"   GE: HS6 {hs6_code} × {multiplier} "
                  f"({mask.sum():,} rows adjusted)")
    return df
...
def build_regions(df: pd.DataFrame, trade_type: str,
    ...
    sub = assign_crop_year(sub, crop_start)
    sub["Region"] = sub["Country"].str.strip().map(REGION_MAP).fillna("Other")
...
def main():
    ...
    for commodity, cfg in COMMODITY_CONFIG.items():
        ...
        # Apply Grain Equivalent multipliers (no-op for most commodities)
        df_c = apply_ge_multipliers(df_c, cfg)
