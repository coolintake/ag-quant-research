...
from config.commodities import normalize_commodity, get_crop_year
...
def _apply_unit_scaling(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_scaled["Scaled_Value"] = df_scaled["Value"].astype(float)  # int64 -> float: dividing an all-integer table raised TypeError
...
def _get_crop_year(date: pd.Timestamp, commodity: str) -> int:
    """Delegates to config.commodities (single source of truth)."""
    return get_crop_year(date, commodity)


def _normalize_commodity_name(name: str) -> str:
    """Delegates to config.commodities (single source of truth).
    'All wheat' / 'Wheat, all' now normalize to 'Wheat, all', NOT 'Wheat',
    so the all-wheat aggregate can no longer be picked up as Wheat ex-durum."""
...
def ingest_statcan_folder(folder_key: str, mapping_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    ...
    # Add crop year.
    # Planting (32100359) is annual with REF_DATE 'YYYY' -> YYYY-01-01. The
    # harvest of calendar year YYYY belongs to crop year YYYY (e.g. 2002-03),
    # so use the calendar year directly. Month-based logic would put it in
    # YYYY-1.
    if folder_key == "planting":
