...
class SurfacePresenter:
    ...
    def _ensure_market_data_dir():
        """Helper to ensure the market_data output folder exists."""
    ...
    def generate_plots(df, spline, metrics=None):
        ...
        # Save to disk first, then show locally
        SurfacePresenter._ensure_market_data_dir()
        fig.write_image("market_data/volatility_surface.png", width=1920, height=1080)
        fig.show()
    ...
    def plot_residuals_trading_map(df, spline):
        ...
        # Save to disk first, then show locally
        SurfacePresenter._ensure_market_data_dir()
        fig.write_image("market_data/residuals_trading_map.png", width=1920, height=1080)
        fig.show()
    ...
    def plot_smile_dashboard(df, spline):
        ...
        # Save to disk first, then show locally
        SurfacePresenter._ensure_market_data_dir()
        fig.write_image("market_data/volatility_smile_dashboard.png", width=1920, height=1080)
        fig.show()
    ...
    def plot_term_structure(clean_df, spline):
        ...
        # Save to disk first, then show locally
        SurfacePresenter._ensure_market_data_dir()
        fig.write_image("market_data/term_structure.png", width=1920, height=1080)
        fig.show()
    ...
    def analyze_smile_skew(clean_df, spline):
        ...
        for exp in expiries:
            ...
            put_iv_10pct = float(spline.ev(0.90, dte))   
            call_iv_10pct = float(spline.ev(1.10, dte))  
            atm_iv = float(spline.ev(1.0, dte))          
        ...
        # Save to disk first, then show locally
        SurfacePresenter._ensure_market_data_dir()
        fig.write_image("market_data/volatility_skew_analysis.png", width=1920, height=1080)
        fig.show()
