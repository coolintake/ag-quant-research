...
graceful degradation. Includes automated macro proxy discovery via CME SOFR futures.
"""
...
import numpy as np
import pandas as pd
from ib_insync import IB, Future, FuturesOption, ContFuture
...
class CornDataHarvester:
    ...
    async def fetch_live_ibkr_risk_free_rate(self) -> float:
        """
        Fetches the current market-implied risk-free rate using front-month 
        3-Month SOFR Futures (SOFR3) on CME. Drops back to config fallback if unavailable.
        """
        try:
            if not self.ib.isConnected():
                logger.warning("IBKR not connected. Using static fallback interest rate.")
                return CONFIG['risk_free_rate']

            # Target the CME 3-Month SOFR Continuous Future
            contract = ContFuture(symbol='SOFR3', exchange='CME', currency='USD')
            qualified = await self.ib.qualifyContractsAsync(contract)
            
            if not qualified:
                logger.warning("Could not qualify SOFR (SOFR3) contract. Using fallback interest rate.")
                return CONFIG['risk_free_rate']
                
            ticker = self.ib.reqMktData(qualified[0], '', snapshot=True)
            await asyncio.sleep(1.0)  # Wait for snapshot payload to fill
            
            price = (ticker.bid + ticker.ask) / 2 if ticker.bid > 0 and ticker.ask > 0 else ticker.last
            if np.isnan(price) or price <= 0:
                price = ticker.close

            if price and not math.isnan(price) and price > 0:
                # SOFR Implied Yield calculation: (100 - Price) / 100
                live_rate = (100.0 - price) / 100.0
                
                # Sanity guard: Check if rate falls within an economically logical window
                if 0.0 <= live_rate <= 0.15:
                    logger.info(f"Live risk-free rate (SOFR Implied): {live_rate:.4%} (Sourced from SOFR3 Price: {price:.2f})")
                    return float(live_rate)
                
            logger.warning("SOFR price data invalid or empty. Using fallback interest rate.")
            return CONFIG['risk_free_rate']
            
        except Exception as e:
            logger.error(f"Failed to fetch live SOFR interest rate: {e}. Using fallback.")
            return CONFIG['risk_free_rate']
    ...
    async def get_option_market_data(self, futures_prices):
        ...
        for conId, info in futures_prices.items():
            ...
            try:
                ...
                for s in strikes:
                    # Skip invalid strikes to prevent flooding the TWS API with bad requests
                    if s % 5 != 0:
                        continue
