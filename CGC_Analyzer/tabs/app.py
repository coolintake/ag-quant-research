...
# Top partner banner -- deliberately a self-contained dark-slate box with a
# FIXED background/text color pair (not Streamlit's own theme variables),
# so it looks identical and stays legible whether the surrounding app is
# in Light or Dark mode, rather than trying to track the current theme.
# Built as ONE continuous string with NO embedded newlines/indentation --
# see the header block further down for why: a multi-line indented string
# here risks Streamlit's Markdown parser treating it as a literal code
# block instead of raw HTML, even with unsafe_allow_html=True.
_TOP_BANNER_HTML = (
    '<div style="background-color:#1e293b; color:#e2e8f0; padding:12px 20px; '
    'border-radius:10px; margin-bottom:15px; font-size:0.95rem; line-height:1.4;">'
    'Analytics by Chilmeran Research | Grain &amp; Oilseeds reports and macro commentary: visit '
    '<a href="https://www.chilmeranresearch.com" target="_blank" rel="noopener noreferrer" '
    'style="color:#60a5fa; text-decoration:none; font-weight:600;">➔ chilmeranresearch.com</a>'
    '</div>'
)
st.markdown(_TOP_BANNER_HTML, unsafe_allow_html=True)
...
with st.sidebar:
    # Matching brand block, same dark-slate/light-text/bright-blue-link
    # treatment as the top banner, for the same cross-theme legibility
    # reason -- the sidebar's own background changes between Light and
    # Dark mode, so a self-contained fixed-color box sidesteps that rather
    # than trying to track it. Also built as one continuous string with no
    # embedded newlines, for the same reason as the top banner above.
    _SIDEBAR_BRAND_HTML = (
        '<div style="background-color:#1e293b; color:#e2e8f0; padding:12px 14px; '
        'border-radius:10px; margin-bottom:14px;">'
        '<div style="font-size:1.05rem; font-weight:700; margin-bottom:4px;">Chilmeran Research</div>'
        '<div style="font-size:0.82rem; color:#94a3b8; margin-bottom:8px; line-height:1.3;">'
        'Grain &amp; Oilseeds research and physical commodity intelligence.'
        '</div>'
        '<a href="https://www.chilmeranresearch.com" target="_blank" rel="noopener noreferrer" '
        'style="font-size:0.85rem; color:#60a5fa; text-decoration:none; font-weight:600;">'
        '➡ Visit Publication (chilmeranresearch.com)</a>'
        '</div>'
    )
